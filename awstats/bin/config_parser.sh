...
#          v1.2.8 - COMPLETE REWRITE to fix syntax errors and duplicate functions
#                    Clean structure, single validate_config function
#                    Fixed all variable expansion issues
...
fi "$config_file" | sed 's/^\[\(.*\)\]$/\1/' | grep -v '^global
...
# Function to validate configuration file - CLEAN SINGLE VERSION
validate_config() {
    local config_file="${CONFIG_FILE}"
    local errors=0
    local warnings=0
    ...
    # Validate global settings with proper variable expansion
    local required_global_keys=("database_file" "logs_dir" "awstats_bin")
    for key in "${required_global_keys[@]}"; do
        local value=$(get_section_value "$config_file" "global" "$key")
        if [[ -z "$value" ]]; then
            ...
            # Expand variables and show actual paths
            local expanded_value=$(expand_config_variables "$value")
            print_color "$GREEN" "✅ Global $key: $expanded_value"
        ...
        for domain in "${domains[@]}"; do
            ...
            if [[ -n "$servers" ]]; then
                ...
                for server in "${server_list[@]}"; do
                    ...
                    # Check server settings with proper variable expansion
                    local server_enabled=$(get_config_value "enabled" "$server" "$domain")
                    local log_directory=$(get_config_value "log_directory" "$server" "$domain")
                    ...
                    if [[ -z "$log_directory" ]]; then
                        ...
                        # Check if log directory exists (already expanded by get_config_value)
                        if [[ ! -d "$log_directory" ]]; then
                            print_color "$YELLOW" "    ⚠️  Log directory doesn't exist: $log_directory"
                            ((warnings++))
                        else
                            print_color "$GREEN" "    ✅ Log directory: $log_directory"
                        fi
                    fi
                done
            fi
...
show_config_overview() {
    ...
    # Global settings with proper variable expansion
    print_color "$CYAN" "🌍 Global Settings:"
    local global_keys=($(get_section_keys "global"))
    for key in "${global_keys[@]}"; do
        local value=$(get_section_value "$config_file" "global" "$key")
    ...
    for domain in "${domains[@]}"; do
        ...
        # Show server details with proper variable expansion
        if [[ -n "$servers" ]]; then
            IFS=',' read -ra server_list <<< "$servers"
            for server in "${server_list[@]}"; do
                server=$(echo "$server" | xargs)
...
test_server_config() {
    ...
    # Test log directory with proper variable expansion
    local log_directory=$(get_config_value "log_directory" "$server_name" "$domain_name")
    if [[ -n "$log_directory" ]]; then
        if [[ -d "$log_directory" ]]; then
            print_color "$GREEN" "✅ Log directory exists: $log_directory"
